...
/**
 * The dialog showing the lengths of a set of non-edges in the current linkage.
 * Used for showing the completeCayleyDistanceVector
 */
public class EdgeLengthContainer extends JDialog implements MouseListener {
	...
	public void refresh(TDLinkageModel t) {
		td = t;
	...
	public EdgeLengthContainer(TDLinkageModel t) {
		// main = m;
		// graph = g;
		...
		for (int i = 0; i < 3; ++i) {
			confirmedProjectionEdges[i] = t.getCompleteCayleyVector().get(i);
	...
	public void paint(Graphics g) {
		...
		int i = 0;
		for (Edge e : td.getCompleteCayleyVector()) {
			double distance = td.getCurRealization().length(e);
	...
	private int pick(int edgeIndex) {
		...
		Edge e = td.getCompleteCayleyVector().get(edgeIndex);
