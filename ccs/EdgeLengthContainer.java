...
public class EdgeLengthContainer extends JDialog implements MouseListener {
	...
	public void refresh(TreeDecomp t) {
		td = t;
	...
	public EdgeLengthContainer(TreeDecomp t) {
		// main = m;
		// graph = g;
		...
		for (int i = 0; i < 3; ++i) {
			confirmedProjectionEdges[i] = t.getCanonicalBaseNonedges().get(i);
	...
	public void paint(Graphics g) {
		...
		int i = 0;
		for (Edge e : td.getCanonicalBaseNonedges()) {
			double distance = td.graph.distance(e);
	...
	private int pick(int edgeIndex) {
		...
		Edge e = td.getCanonicalBaseNonedges().get(edgeIndex);
