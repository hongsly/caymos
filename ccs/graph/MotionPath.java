...
public class MotionPath extends ContinuousMotion {

	public MotionPath(TreeDecomp t) {
		super(t);
	...
	public static MotionPath findPath(TreeDecomp t, Graph g1, Graph g2) {
		Edge baseNonEdge = t.getBaseNonedge();
		double l1 = g1.distance(baseNonEdge), l2 = g2.distance(baseNonEdge);
		SolutionType startType = t.getForwardSolutionType(g1);
		SolutionType targetType = t.getForwardSolutionType(g2);
		OrientedCCS startO = t.ccs.getOrientedCCS(startType);
		OrientedCCS targetO = t.ccs.getOrientedCCS(targetType);
		...
		if (endIndex >= 0) {
			List<Node> nodesOnPath = component.subList(0, endIndex + 1);
			MotionPath p = new MotionPath(t);
			p.addAll(nodesOnPath);

			Node start = p.startNode();
			Node newStart = new Node(l1, start.getEnd(), start.getInterval(),
					start.getSolutionType());
			p.set(0, newStart);
			Node end = p.endNode();
			Node newEnd = new Node(end.getStart(), l2, end.getInterval(),
					end.getSolutionType());
