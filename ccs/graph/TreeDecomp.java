...
public class TreeDecomp {
	...
	public double getBaseNoneedgeLength() {
		return graph.distance(baseNonEdge);
	}
	...
	public AbstractList<Edge> getCanonicalBaseNonedges() {
		boolean b = isLow();
		assert (b);
		return fourcycleInfo.getCanonicalBaseNonedges();
	}
	...
	// ===================== orientations ======================
	private int orientationOf(ConstructionStep s, Graph g) {
	...
	public SolutionType getForwardSolutionType(Graph g, int numOfSteps) {
		...
		SolutionType forward = new SolutionType(getNumOfConstructStep());
	...
	public TreeDecomp getExtremeGraph(int index) {
		...
		Graph eg = graph.inducedSubgaph(vset);
		TreeDecomp t = new TreeDecomp(eg);
	...
	public ArrayList<Graph> solve(double length) {
	 ...
	 */
	public ArrayList<Graph> solve(EdgePos newBasePos) {
	 ...
	 */
	private boolean solve(ArrayList<Graph> solutions, int stepIndex,
			Graph curGraph) {
		...
		double r1 = getEdgeLength(v1, v);// graph.distance(v1, v);
		double r2 = getEdgeLength(v2, v);// graph.distance(v2, v);
	 ...
	 */
	public Graph solve(double length, SolutionType forward) {
		Point2D p1 = graph.getPoint(baseNonEdge.v1());
		Point2D p2 = new Point2D(p1.x() + length, p1.y());
	...
	// solve w/ sol type
	private Graph solve(EdgePos newBasePos, SolutionType forward) {
		if (!constructionSequenceGenerated()) {
			generateConstructionSequence();
			this.normalizeBaseNonEdge();
		}
	...
	public Graph tryRealize(SolutionType t) {
		Graph g = tryRealize(getBaseNoneedgeLength(), t);
		if (g != null)
			return g;

		boolean b = ccsGenerated();
		assert (b);
		OrientedCCS occs = ccs.getOrientedCCS(t);
		if (occs != null) {
	...
	// add accuracy
	public ArrayList<Graph> tryRealize(double length) {
		ArrayList<Graph> solutions = solve(length);
	...
	public boolean realizable(double length) {
		ArrayList<Graph> solutions = tryRealize(length);
		return !solutions.isEmpty();
	}

	public boolean realizable(double length, SolutionType forward) {
		Graph g = tryRealize(length, forward);
		return g != null;
	}
	 ...
	 */
	private double iterateTry(SolutionType t, double length, int direction) {
	...
	public void generateCCS() {
		ccs = CCS.generateCCS(this);
	}

	// assume candidates is sorted
	OrientedCCS genOrientedCCSFromCandidates(ArrayList<Double> candidates,
			SolutionType type) {
		OrientedCCS ccs = new OrientedCCS(type);
		...
		if (realizable(first, type)) {
			if (realizable((first + second) / 2, type)) { // first is endpoint
				lastEndpoint = first;
				if (Graph.DEBUG)
					System.out.println(first + " is interval start");
			} else { // first is isolated point
				ccs.appendInterval(first, first);
				if (Graph.DEBUG)
					System.out.println(first + "is isolated point");
			}
		} else
			System.out.println(first + " not realizable");
		...
		for (int i = 1; i < candidates.size() - 1; ++i) {
			...
			if (!realizable(cur, type)) {
				System.out.println(cur + " not realizable");
		...
		if (realizable(last, type)) {
			...
			System.out.println(last + " not realizable");
	...
	public MotionPath findPath(double l1, double l2) {
		MotionPath result = null;
		ArrayList<Graph> solutions1 = tryRealize(l1);
		Graph g = tryRealize(l1, getForwardSolutionType());
		if (g != null)
			solutions1.add(0, g);
		ArrayList<Graph> solutions2 = tryRealize(l2);
		g = tryRealize(l2, getForwardSolutionType());
		if (g != null)
			solutions2.add(0, g);
		for (Graph g1 : solutions1)
			for (Graph g2 : solutions2) {
				MotionPath path = findPath(g1, g2);
				if (path != null) {
	...
	public MotionPath findPath(Graph g1, Graph g2) {
		return MotionPath.findPath(this, g1, g2);
	}

	public ConnectedComponent findPathFrom(Graph g1) {
		return ConnectedComponent.findComponentOf(this, g1);
	}
	...
	public double completeDistance(Graph g1, Graph g2) {
		...
		// sqrt( sum[(ai - bi)^2] )
		double sum = 0;
		for (Edge e : getCanonicalBaseNonedges()) {
			double l1 = g1.distance(e);
			double l2 = g2.distance(e);
			sum += (l1 - l2) * (l1 - l2);
		}
