...
public class GPanel extends JPanel implements MouseListener,
	 ...
	 */
	private void drawBase(Graph graph, Graphics2D g2d) {
	...
	private void drawEdgeLength(Graph graph, Edge e, Graphics2D g2d) {
		drawEdgeLength(graph, e.v1(), e.v2(), g2d);
	}

	private void drawEdgeLength(Graph graph, Vertex v, Vertex u, Graphics2D g2d) {
	...
	private void drawVertex(Graph graph, Vertex v, Graphics2D g2d) {
	...
	private void drawEdge(Graph graph, Vertex v, Vertex u, Graphics2D g2d) {
		Point2D pv = graph.getPoint(v), pu = graph.getPoint(u);
		if (control.isDrawing()) {
	...
	private void drawEdge(Graph graph, Edge e, Graphics2D g2d) {
		drawEdge(graph, e.v1(), e.v2(), g2d);
	}

	private void drawGraph(Graph graph, Graphics2D g2d) {
		...
		for (int i = 0; i < graph.getSize(); ++i) {
			...
			for (Vertex u : graph.getNeighbors(v)) {
				if (control.isShowingCluster()) {
					...
					Color c = Color.red;
					if (tdModel.isTd()) {
						SolutionType t = tdModel.getForwardSolutionType(graph);
	...
	private void drawTrace(Graphics2D g2d) {
		...
		for (int index = 0; index < traceModel.getTracingVertices().size(); ++index) {
			...
			if (!traceCreated) {
				Trace trace = new Trace(tracingVertex, tdModel.getTd().ccs
	...
	public void paint(Graphics g) {
		...
		Graph start = motionModel.getStartRealization();
		if (start != null) {
			if (pathStart == null) {
		...
		Graph end = motionModel.getEndRealization();
		if (end != null) {
			if (pathEnd == null) {
		...
		if (control.isTracingMotion() == 3) {
			...
			Graph g1 = motionModel.getSpinnerModel1().getValue().getValue();
			...
			Graph g2 = motionModel.getSpinnerModel2().getValue().getValue();
	...
	private void drawCompleteCayley(Graphics2D g2d) {
		for (Edge e : tdModel.getTd().getCanonicalBaseNonedges()) {
